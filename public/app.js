...
async function buscarMeta(id, nome, plataforma) {
    ...
    try {
        const res = await fetch(`${API}/roms/${id}/metadados`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function buscarMetaTodos() {
    ...
    for (const rom of semMeta) {
        try {
            const res = await fetch(`${API}/roms/${rom.id}/metadados`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
async function salvarCapa(id) {
    ...
    try {
        const res  = await fetch(`${API}/roms/${id}/capa`, {
            method:  'PATCH',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (data.sucesso) {
            ...
            const card = document.getElementById(`card-${id}`);
            if (card) {
                card.querySelector('.card-capa').src = url;
            }
